...
class TurcEstimator:
    ...
    def _process_basin(self, gauge_id: str, df: pl.DataFrame) -> pl.DataFrame:
        ...
        annual = basin_df.group_by("year").agg(
            [
        ...
        for row in annual.iter_rows(named=True):
            turc_values = compute_turc_discharge(
                row["annual_precip_mm"], row["annual_temp_c"]
            )
    ...
    def _process_batch(self, gauge_ids: list[str]) -> pl.DataFrame:
        ...
        for gauge_id in gauge_ids:
            basin_annual = self._process_basin(gauge_id, df)
    ...
    def compute_annual(self) -> pl.DataFrame:
